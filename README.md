# 學習數據分析儀表板 · Learning Analytics Dashboard
...
- **圖表**：[Chart.js 4.4.0](https://www.chartjs.org/) + chartjs-plugin-annotation 3.0.1
- **PWA**：Service Worker（App Shell Cache First + Data Network First）、iOS/Android 安裝支援
- **資料層**：`behavior-loader.js` 負責 lazy load JSON，`filter-engine.js` 處理多維度篩選邏輯
...
`at_risk_profile.json` 須符合 schema version ≥ 2.0；多學期資料請使用 schema 3.0（`by_semester` 結構）。
